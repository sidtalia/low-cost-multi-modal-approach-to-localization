...
def get_Intermediate_Points(slope1, slope2, X1, X2, Y1, Y2):
	...
	if(d>20):
		d = 20
	int1[0] = X1 + 0.4*m.cos(slope1*DEG2RAD)*d
	int1[1] = Y1 + 0.4*m.sin(slope1*DEG2RAD)*d
	int2[0] = X2 - 0.4*m.cos(slope2*DEG2RAD)*d
	int2[1] = Y2 - 0.4*m.sin(slope2*DEG2RAD)*d
	return int1,int2
...
def get_path_length(X1,Y1,X2,Y2,X3,Y3,X4,Y4):
...
def s_k(X1, Y1, slope1, X2, Y2, slope2):
	int1,int2 = get_Intermediate_Points( slope1, slope2, X1, X2, Y1, Y2)
	t = get_T( 0, X1, Y1, int1[0], int1[1], int2[0], int2[1], X2, Y2, 2e-2)
	kappa = get_Curvature(X1, Y1, int1[0], int1[1], int2[0], int2[1], X2, Y2,t)
	kappa = np.max(kappa)
	s = get_path_length(X1, Y1, int1[0], int1[1], int2[0], int2[1], X2, Y2)*0.5
	s*=s
	return m.fabs(s*kappa)
